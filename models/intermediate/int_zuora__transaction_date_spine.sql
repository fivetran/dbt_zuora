...
with spine as (
    ...
    {% set first_date_query %}
        select  min( first_charge_date ) as min_date from {{ ref('zuora__billing_history') }}
    {% endset %}
    ...
    {% if execute %}
    {% set last_date_query %}
        select  max( most_recent_charge_date ) as max_date from {{ ref('zuora__billing_history') }}
    {% endset %}
